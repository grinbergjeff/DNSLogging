...
for index, time in enumerate(timeStampMS):
	...
	subtraction = abs(time - timeStampMS[checkme] )
	if subtraction >= 60000:
			# Look for what exists in the link and filter it
			# Typical links have 3 dots:
		if siteLink[index].count('.') == 3:
...
# Take the lengths between each query call and display all the links that occured after each call for each respective one.
counter = 0
